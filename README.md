...
Each network enforces replay protection at the smart-contract level. On EVM the `Bridge` uses transaction-id mappings for `FundsIn` and per-selector sequential nonces for `FundsOut`.
